...
class LocalizationNetwork(nn.Module):
    ...
    def __init__(self, F, I_channel_num):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(64, 128, 3, 1, 1, bias=False), nn.BatchNorm2d(
                128), nn.ReLU(True),
            nn.MaxPool2d(2, 2),  # batch_size x 128 x I_height/4 x I_width/4
            nn.Conv2d(128, 256, 3, 1, 1, bias=False), nn.BatchNorm2d(
                256), nn.ReLU(True),
            nn.MaxPool2d(2, 2),  # batch_size x 256 x I_height/8 x I_width/8
            nn.Conv2d(256, 512, 3, 1, 1, bias=False), nn.BatchNorm2d(
                512), nn.ReLU(True),
...
class GridGenerator(nn.Module):
    ...
    def _build_P_hat(self, F, C, P):
        ...
        rbf = np.multiply(np.square(rbf_norm), np.log(
            rbf_norm + self.eps))  # n x F
