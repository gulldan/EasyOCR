...
class UnitTest:
    def __init__(self,
        ...
        if not os.path.isfile(easy_ocr_init):
            raise FileNotFoundError(
                "Invalid easyocr_module. The directory should contain __init__.py.")
        ...
        if not hasattr(self.easyocr, 'utils'):
            setattr(self.easyocr, 'utils',
                    importlib.import_module('easyocr.utils'))
        if not hasattr(self.easyocr, 'detection'):
            setattr(self.easyocr, 'detection',
                    importlib.import_module('easyocr.detection'))
        if not hasattr(self.easyocr, 'recognition'):
            setattr(self.easyocr, 'recognition',
                    importlib.import_module('easyocr.recognition'))

        self.easyocr_dir = os.path.dirname(easyocr.__file__)

        print("Unit test is set for EasyOCR at {}".format(
            os.path.abspath(self.easyocr_dir)))
    ...
    def set_data(self, test_data):
        ...
        english_mini_bgr, english_mini_gray = self.easyocr.utils.reformat_input(
            images['english']['mini'])
        english_small_bgr, english_small_gray = self.easyocr.utils.reformat_input(
            images['english']['small'])
        ...
        setattr(self.inputs, 'easyocr_config', self.dict2attr(
            solution_book['inputs']['easyocr_config']))

    def dict2attr(self, dict_):
        attr = Attributes()
        [setattr(attr, key, self.dict2attr(value)) if isinstance(
            value, dict) else setattr(attr, key, value) for (key, value) in dict_.items()]
        return attr
    ...
    def validate(self, test, solution, dtype):
        if dtype == str:
            ...
            raise TypeError(
                "Unsupport data type ({}) to validate. Supporting types are str, int, float, dict, list, np.ndarray, or torch.Tensor".format(dtype))
    ...
    def validate_all(self, results, solutions, dtypes):
        ...
        for (result, solution, dtype) in zip(results, solutions, dtypes):
            if (not self.is_list_or_tuple(result)
                ...
                validation.append(self.validate(
                    result, solution, type(solution)))
            elif(self.is_list_or_tuple(result)
                 and self.is_list_or_tuple(result)
                 and self.is_list_or_tuple(result)
                 ):
                validation.append(self.validate_all(
                    results, solutions, type(solution)))
            else:
                raise
    ...
    def do_test(self, verbose=None):
        ...
        for name, tests in self.test_book.items():
            ...
            num_passed = 0
            min_pass = sum(
                [test['severity'] == 'Error' for test in tests.values()])
            if self.verbose > 0:
                print("##Testing module {}: {:d} tests will be performed.".format(
                    name, num_test))
            for test_id, test in tests.items():
                ...
                test_result = self.validate(
                    results, test['output'], type(test['output']))
                if test_result:
                    num_passed += 1
                    if self.verbose > 1:
                        print(
                            "#### Passed. [{:d}/{:d}]".format(num_passed, num_test))
                else:
                    if test['severity'] == "Warning":
                        ...
                        if self.verbose > 2:
                            print(
                                "##### The test yields results ({}) which are different from the expected values ({}).")
